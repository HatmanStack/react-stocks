...
jest.unstable_mockModule('../../../backend/src/utils/dynamodb.util.js', () => ({
  dynamoDb: {
    send: jest.fn(),
  },
...
const { calculateTTLByDataType } = await import('../../../backend/src/utils/cache.util.js');
const { putStock, batchPutStocks } = await import('../../../backend/src/repositories/stocksCache.repository.js');
const { dynamoDb, batchPutItems } = await import('../../../backend/src/utils/dynamodb.util.js');
