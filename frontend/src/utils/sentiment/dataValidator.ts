...
export function validateCombinedData(
  ...
  const latestDate = data.length > 0
    ? data.reduce((a, b) => a.date > b.date ? a : b).date
    : null;
  ...
  if (data.length >= minRecords && !hasGoodCoverage) reasons.push(`low coverage (${(coverageRatio * 100).toFixed(0)}%)`);
  if (data.length >= minRecords && !isFresh) reasons.push(`stale (latest: ${latestDate || 'none'})`);
...
export function validateArticleData(
  ...
  const isAcceptable = data.length >= minRecords && hasPublisherData && hasPhase5Data && hasGoodCoverage && isFresh;
