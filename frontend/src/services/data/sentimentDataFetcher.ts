...
import { getSentimentResults, getArticleSentiment, fetchLambdaNews, triggerSentimentAnalysis } from '@/services/api/lambdaSentiment.service';
...
export async function fetchCombinedSentiment(
  ...
  if (quality.isAcceptable) {
    console.log(`[SentimentFetcher] Using ${localData.length} local records for ${ticker} (coverage: ${(quality.coverageRatio * 100).toFixed(0)}%)`);
    return localData;
  }
  ...
  if (Environment.USE_LAMBDA_SENTIMENT) {
    ...
    try {
      ...
      if (lambdaResults.dailySentiment.length > 0) {
        console.log(`[SentimentFetcher] Backend returned ${lambdaResults.dailySentiment.length} records`);
    ...
    console.log(`[SentimentFetcher] No backend configured, using ${localData.length} local records`);
  }
...
export async function fetchArticleSentiment(
  ...
  if (quality.isAcceptable) {
    console.log(`[SentimentFetcher] Using ${localData.length} local articles for ${ticker} (coverage: ${(quality.coverageRatio * 100).toFixed(0)}%)`);
    return localData;
  }

  // Step 2: Fall back to backend API
  if (Environment.USE_LAMBDA_SENTIMENT) {
    console.log(`[SentimentFetcher] Articles insufficient: ${quality.reasons.join(', ')}. Fetching backend.`);

    try {
      const lambdaResults = await getArticleSentiment(ticker, startDate, endDate);

      if (lambdaResults.articles.length > 0) {
        console.log(`[SentimentFetcher] Backend returned ${lambdaResults.articles.length} articles`);
